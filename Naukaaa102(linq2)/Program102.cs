...
var groupStudents2 = Student.GetStudents()
                    .GroupBy(x => new {x.Gender, x.Age, x.Salary}); 
// or just with anonymous type: select(x => new { Gender = x.key.gender}), to not write it later
foreach (var item in groupStudents2)
{
    Console.WriteLine(item.Key.Gender + " " + item.Key.Age + " " + item.Sum(x => x.Salary)); 
}
